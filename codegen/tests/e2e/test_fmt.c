...
static void
assert_fmt_uint(jf_acc_t val, const char *expect)
{
    uint8_t     sz;
    char        buf[20];
...
static void
assert_fmt_int(jf_sacc_t val, const char *expect)
{
    uint8_t     sz;
    char        buf[20];
...
{
    assert_fmt_uint((jf_acc_t)UINT32_MAX, "4294967295");
}
...
{
    assert_fmt_int((jf_sacc_t)INT32_MIN, "-2147483648");
}
