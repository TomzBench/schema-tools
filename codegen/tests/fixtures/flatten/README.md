...
annotated object (`x-jsmn-generate`) can be discovered. Combinational keywords
...
| ------ | ---------------------------------- | ------------------------------------------------ |
| **A**  | `type: object` + `x-jsmn-generate` | Record struct, recurse into `properties`         |
| **B**  | `type: array`  + `x-jsmn-generate` | Record CArray, recurse into `items`              |
