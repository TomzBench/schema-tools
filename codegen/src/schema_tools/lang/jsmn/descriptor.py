...
# nameblob[]
# st_array[]   comment, kind, min, max, elem_size, desc_idx
# st_field[]   comment, off_name, off_value, off_present, desc_idx
# st_struct[]  comment, nfields, size, ntoks, field0
# ST_{{struct.name | upper}}_IDX {{idx}}
# ST_{{struct.name | upper}}_TOK {{idx}}
...
class StructDescriptor(Descriptor):
    """All context for rendering a struct entry in a struct st_struct"""
...
class FieldDescriptor(Descriptor):
    """All context for rendering a field entry in a struct st_field"""
...
class ArrayDescriptor(Descriptor):
    """All context for rendering an array entry in a struct st_array"""
