...
_PRIM_KINDS: dict[Primitive, str] = {
    Primitive.BOOL: "ST_KIND_BOOL",
    Primitive.CHAR: "ST_KIND_CHAR",
    Primitive.UINT8: "ST_KIND_U8",
    Primitive.INT8: "ST_KIND_I8",
    Primitive.UINT16: "ST_KIND_U16",
    Primitive.INT16: "ST_KIND_I16",
    Primitive.UINT32: "ST_KIND_U32",
    Primitive.INT32: "ST_KIND_I32",
    Primitive.UINT64: "ST_KIND_U64",
    Primitive.INT64: "ST_KIND_I64",
    Primitive.FLOAT: "ST_KIND_FLOAT",
    Primitive.DOUBLE: "ST_KIND_DOUBLE",
}
assert _PRIM_KINDS.keys() == set(Primitive), "missing primitive kind"

_ARRAY_KINDS: dict[ArrayKind, str] = {
    ArrayKind.FIXED: "ST_KIND_FIXED",
    ArrayKind.STRING: "ST_KIND_STRING",
    ArrayKind.VLA: "ST_KIND_VLA",
}
...
def filters(
    ...
    def _resolve_ctype(ctype: CType) -> str:
        try:
            return f"ST_PRIM({_PRIM_KINDS[Primitive(ctype.name)]})"
        except ValueError:
            return f"ST_STRUCT({struct_index[ctype.name]})"

    def _resolve_ref(ref: Key | CType) -> str:
        if isinstance(ref, CType):
            return _resolve_ctype(ref)
        return f"ST_ARRAY({ref.pos})"
