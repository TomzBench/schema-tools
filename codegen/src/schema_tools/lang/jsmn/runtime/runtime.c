...
#ifdef ST_HAS_INT64
typedef uint64_t st_acc_t;
typedef int64_t  st_sacc_t;
#define ST_ACC_MAX UINT64_MAX
#else
typedef uint32_t st_acc_t;
typedef int32_t  st_sacc_t;
#define ST_ACC_MAX UINT32_MAX
#endif
...
static int
tok_raw_int(const char *json, const jsmntok_t *tok, st_acc_t *out, bool *neg)
{
    ...
    *neg = false;
    ...
    while (c < end) {
        ...
        st_acc_t d = (st_acc_t)(*c - '0');
        if (val > (ST_ACC_MAX - d) / 10) {
            return -1; // overflow
        }
        val = val * 10 + d;
        c++;
...
ST_UINT32_TYPES(GEN_UINT_PARSER)
ST_INT32_TYPES(GEN_INT_PARSER)

#ifdef ST_HAS_INT64
ST_UINT64_TYPES(GEN_UINT_PARSER)
ST_INT64_TYPES(GEN_INT_PARSER)
#endif
...
static uint16_t
calculate_array_stride(const struct st_schemas *schema,
                       const struct st_array   *a)
{
    // TODO this is able to be codegenned into the descriptors
    if (a->elem_size != 0) {
        return a->elem_size;
    } else {
        assert(ST_IS_ARRAY(a->elem));
        const struct st_array *child = &schema->arrays[ST_IDX(a->elem)];
...
struct decoder {
    const struct st_schemas *schemas;
    const char              *json;
    jsmntok_t               *toks;
};

static int
decode_type(const struct decoder *dec, void *dst, int itok, st_type_t type);
...
{
    const struct st_array *a = &dec->schemas->arrays[arr_idx];
    if (a->kind == ST_KIND_STRING) {
...
{
    const struct st_struct *s = &dec->schemas->structs[struct_idx];
    ...
    for (int fidx = s->field0; fidx < s->field0 + s->nfields; fidx++) {
        const struct st_field *f = &dec->schemas->fields[fidx];
...
{
    if (ST_IS_ARRAY(type)) {
        return decode_array(dec, dst, ST_IDX(type), itok);
    } else if (ST_IS_STRUCT(type)) {
        return decode_struct(dec, dst, ST_IDX(type), itok);
    } else {
...
const char *
fmt_uint(st_acc_t val, char (*buf)[20], uint8_t *sz)
{
    *sz = 0;
    do {
...
const char *
fmt_int(st_sacc_t val, char (*buf)[20], uint8_t *sz)
{
    if (val >= 0) {
        return fmt_uint((st_acc_t)val, buf, sz);
    } else {
        const char *result = fmt_uint((st_acc_t)(-(val + 1)) + 1, buf, sz);
        (*buf)[20 - ++*sz] = '-';
        return (const char *)&(*buf)[20 - *sz];
    }
}
...
static int
encode_type(struct encoder *enc, void *src, st_type_t type);
...
{
    const struct st_array *a = &enc->schemas->arrays[idx];
    if (a->kind == ST_KIND_STRING) {
...
{
    const struct st_struct *s = &enc->schemas->structs[idx];
    int                     err = -1;
    bool                    first = true;

    cursor_push_safe(enc->cursor, '{');
    for (int fidx = s->field0; fidx < s->field0 + s->nfields; fidx++) {
        const struct st_field *f = &enc->schemas->fields[fidx];
...
{
    if (ST_IS_ARRAY(type)) {
        return encode_array(enc, src, ST_IDX(type));
    } else if (ST_IS_STRUCT(type)) {
        return encode_struct(enc, src, ST_IDX(type));
    } else {
