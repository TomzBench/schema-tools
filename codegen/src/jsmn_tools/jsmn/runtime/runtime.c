...
#ifdef JT_HAS_INT64
typedef uint64_t jt_acc_t;
typedef int64_t  jt_sacc_t;
#define JT_ACC_MAX UINT64_MAX
#else
typedef uint32_t jt_acc_t;
typedef int32_t  jt_sacc_t;
#define JT_ACC_MAX UINT32_MAX
#endif
...
static int
tok_raw_int(const char *json, const jsmntok_t *tok, jt_acc_t *out, bool *neg)
{
    ...
    *neg = false;
    ...
    while (c < end) {
        ...
        jt_acc_t d = (jt_acc_t)(*c - '0');
        if (val > (JT_ACC_MAX - d) / 10) {
            return -1; // overflow
        }
        val = val * 10 + d;
        c++;
...
JT_UINT32_TYPES(GEN_UINT_PARSER)
JT_INT32_TYPES(GEN_INT_PARSER)

#ifdef JT_HAS_INT64
JT_UINT64_TYPES(GEN_UINT_PARSER)
JT_INT64_TYPES(GEN_INT_PARSER)
#endif
...
static uint16_t
calculate_array_stride(const struct jt_schemas *schema,
                       const struct jt_array   *a)
{
    // TODO this is able to be codegenned into the descriptors
    if (a->elem_size != 0) {
        return a->elem_size;
    } else {
        assert(JT_IS_ARRAY(a->elem));
        const struct jt_array *child = &schema->arrays[JT_IDX(a->elem)];
...
struct decoder {
    const struct jt_schemas *schemas;
    const char              *json;
    jsmntok_t               *toks;
};

static int
decode_type(const struct decoder *dec, void *dst, int itok, jt_type_t type);
...
{
    const struct jt_array *a = &dec->schemas->arrays[arr_idx];
    if (a->kind == JT_KIND_STRING) {
...
{
    const struct jt_struct *s = &dec->schemas->structs[struct_idx];
    ...
    for (int fidx = s->field0; fidx < s->field0 + s->nfields; fidx++) {
        const struct jt_field *f = &dec->schemas->fields[fidx];
...
{
    if (JT_IS_ARRAY(type)) {
        return decode_array(dec, dst, JT_IDX(type), itok);
    } else if (JT_IS_STRUCT(type)) {
        return decode_struct(dec, dst, JT_IDX(type), itok);
    } else {
...
const char *
fmt_uint(jt_acc_t val, char (*buf)[20], uint8_t *sz)
{
    *sz = 0;
    do {
...
const char *
fmt_int(jt_sacc_t val, char (*buf)[20], uint8_t *sz)
{
    if (val >= 0) {
...
static int
encode_type(struct encoder *enc, void *src, jt_type_t type);
...
{
    const struct jt_array *a = &enc->schemas->arrays[idx];
    if (a->kind == JT_KIND_STRING) {
...
{
    const struct jt_struct *s = &enc->schemas->structs[idx];
    int                     err = -1;
    bool                    first = true;

    cursor_push_safe(enc->cursor, '{');
    for (int fidx = s->field0; fidx < s->field0 + s->nfields; fidx++) {
        const struct jt_field *f = &enc->schemas->fields[fidx];
...
{
    if (JT_IS_ARRAY(type)) {
        return encode_array(enc, src, JT_IDX(type));
    } else if (JT_IS_STRUCT(type)) {
        return encode_struct(enc, src, JT_IDX(type));
    } else {
        ...
        uint32_t                 n)
{
    ...
    cursor_push_safe(&cursor, '{');
    for (uint32_t i = 0; i < n; i++) {
        if (i) cursor_push_safe(&cursor, ',');

        cursor_push_safe(&cursor, '"');
        cursor_copy_safe(&cursor, parts[i].key, strlen(parts[i].key));
        cursor_push_safe(&cursor, '"');
        cursor_push_safe(&cursor, ':');

        err = encode_type(&enc, (void *)parts[i].data, parts[i].type);
        if (err < 0) return err;
    }
    cursor_push_safe(&cursor, '}');
    return cursor.pos;
          ...
          uint32_t                 n)
{
    ...
    jsmn_init(&parser);
    atoks = jsmn_parse(&parser, src, slen, toks, ntoks);
    if (atoks < 0) return atoks;
    if (atoks < 1) return JT_ERR_PARTIAL;
    if (toks[0].type != JSMN_OBJECT) return JT_ERR_TYPE;

    struct decoder dec = {.schemas = schemas, .json = src, .toks = toks};
    int            nkeys = toks[0].size;
    int            t = 1;
    ...
    for (int k = 0; k < nkeys; k++) {
        ...
        for (uint32_t i = 0; i < n; i++) {
            if (!tok_memcmp(src, &toks[t], parts[i].key,
                            strlen(parts[i].key))) {
                int err = decode_type(&dec, parts[i].data, vtok,
                                      parts[i].type);
                if (err < 0) return err;
                break;
            }
        }
        t = vtok + tok_skip(toks, vtok);
    }
    return parser.pos;
}
